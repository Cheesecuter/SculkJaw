...
public class ModBlocks {
    ...
    public static final DeferredBlock<Block> SCULK_ACID;
    ...
    static {
        SCULK_JAW = MOD_BLOCKS.registerSimpleBlock("sculk_jaw",
                BlockBehaviour.Properties.ofFullCopy(Blocks.SCULK).strength(3.0F, 3.0F).forceSolidOn().noOcclusion());
        CONCENTRATED_SCULK = MOD_BLOCKS.registerSimpleBlock("concentrated_sculk",
                BlockBehaviour.Properties.ofFullCopy(Blocks.SCULK).sound(SoundType.SCULK_CATALYST).strength(3.0F, 3.0F));
        SCULK_ACID = MOD_BLOCKS.registerSimpleBlock("sculk_acid",
                BlockBehaviour.Properties.ofFullCopy(Blocks.WATER).mapColor(MapColor.COLOR_CYAN).replaceable().noCollission().strength(100.0F).pushReaction(PushReaction.DESTROY).noLootTable().liquid().sound(SoundType.EMPTY));
