...
public class SculkJelly extends HalfTransparentBlock {
    ...
    public void fallOn(Level level, BlockState blockState, BlockPos blockPos, Entity entity, float f) {
        if (!(f < 4.0) && entity instanceof LivingEntity livingEntity) {
            LivingEntity.Fallsounds fallsounds = livingEntity.getFallSounds();
            SoundEvent soundEvent = f < 7.0 ? fallsounds.small() : fallsounds.big();
    ...
    public void absorbExperience(Level level, BlockPos blockPos) {
        if (level instanceof ServerLevel serverLevel && level.getBlockState(blockPos).is(this)) {
            ...
            for (int i = 0; i < allDirections.length; ++i) {
                ...
                if (serverLevel.getBlockState(blockPos.relative(direction)).is(ModBlocks.CONCENTRATED_SCULK.get()) &&
                        serverLevel.getBlockState(blockPos.relative(direction).above()).is(ModBlocks.SCULK_JAW.get())) {
                ...
                else if (serverLevel.getBlockState(blockPos.relative(direction)).is(ModBlocks.SCULK_JAW.get()) &&
                        serverLevel.getBlockState(blockPos.relative(direction).below()).is(ModBlocks.CONCENTRATED_SCULK.get())) {
