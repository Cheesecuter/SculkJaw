...
public class UmbraFern extends BushBlock implements BonemealableBlock {
    ...
    public boolean isValidBonemealTarget(LevelReader levelReader, BlockPos blockPos, BlockState blockState) {
        return ModBlocks.LARGE_UMBRAFERN.value().defaultBlockState().canSurvive(levelReader, blockPos) && levelReader.isEmptyBlock(blockPos.above());
    }
    ...
    public void performBonemeal(ServerLevel serverLevel, RandomSource randomSource, BlockPos blockPos, BlockState blockState) {
        if (blockPos.getY() < getMaxY(serverLevel)) {
            DoublePlantBlock.placeAt(serverLevel, ModBlocks.LARGE_UMBRAFERN.value().defaultBlockState(), blockPos, 2);
        }
        else {
