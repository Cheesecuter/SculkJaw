    ...
    public void absorbExperience(Level level, BlockPos blockPos) {
        if(level instanceof ServerLevel serverLevel && level.getBlockState(blockPos).is(this)) {
            ...
            for(int i = 0; i < allDirections.length; ++i) {
                ...
                if(serverLevel.getBlockState(blockPos.relative(direction)).is(ModBlocks.SCULK_AGGREGATOR.get()) &&
                    ...
                    if(serverLevel.getBlockState(blockPos.relative(direction)).getValue(SculkAggregator.COMBINED_WITH_SCULK_JAW)) {
                        ...
                        serverLevel.getBlockState(blockPos.relative(direction).below()).is(ModBlocks.SCULK_AGGREGATOR.get())) {
